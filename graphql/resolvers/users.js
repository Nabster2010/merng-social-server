...
module.exports = {
	Query: {
		...
		verifyToken: (parent, args, context, info) => {
	...
	Mutation: {
		login: async (parent, args, context, info) => {
			...
			const { valid, errors } = validtateLoginInput(username, password);
			if (!valid) {
		...
		register: async (_, args, context, info) => {
			...
			const { errors, valid } = validtateRegisterInput(
				username,
				email,
				password,
				confirmPassword
		...
		upload: async (_, args, context, info) => {
			const user = checkAuth(context);
